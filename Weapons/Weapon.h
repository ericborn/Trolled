...
// declare the classes the weapon will use
class UAnimMontage;
...
// used to keep track of the weapon state
UENUM(BlueprintType)
...
// weapon data, mag size, fire rate
...
// 1st and 3rd person weapon animations
USTRUCT()
struct FWeaponAnim
{
	GENERATED_USTRUCT_BODY()
...
// setup how the engine uses traces to determine where bullets go
// max distance, damage, radius, damage type
USTRUCT(BlueprintType)
struct FHitScanConfiguration
{
	...
	// takes the bone and damage multiplier as input. A value of head, 2 would mean double damage from a headshot
...
UCLASS()
class TROLLED_API AWeapon : public AActor
{
	GENERATED_BODY()

	// allows AMainCharacter to access any function inside weapon even if its private
...
protected:
	...
	// consume ammo from the inventory when reloading
...
// Input
	...
	// RPC, trigger reload from server 
	UFUNCTION(reliable, client)
	void ClientStartReload();

	// check if player can fire or reload
	bool CanFire() const;
	...
	// get current ammo amount mag
	UFUNCTION(BlueprintPure, Category = "Weapon")
	int32 GetCurrentAmmoInMag() const;

	// get mag size 
	int32 GetAmmoPerMag() const;
	...
	class AMainCharacter* GetPawnOwner() const;

	// set the weapon's owning pawn 
	void SetPawnOwner(AMainCharacter* MainCharacter);
...
protected:
	...
	class AMainCharacter* PawnOwner;
...
protected:
	...
	// force feedback effect to play when the weapon is fired, controller vibration
	UPROPERTY(EditDefaultsOnly, Category = Effects)
...
// Replication & effects
	...
	// rep when someone equips a weapon
	UFUNCTION()
...
// Weapon usage
	...
	// handle hit locally before being called to the server
	void HandleHit(const FHitResult& Hit, class AMainCharacter* HitPlayer = nullptr);

	// server verification of hit
	UFUNCTION(Server, Reliable, WithValidation)
	void ServerHandleHit(const FHitResult& Hit, class AMainCharacter* HitPlayer = nullptr);
...
// Weapon usage helpers
	...
	// here in video, down in public in final code
	// find hit 
	FHitResult WeaponTrace(const FVector& StartTrace, const FVector& EndTrace) const;

// in final code, not in video
// public:
